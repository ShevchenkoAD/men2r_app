  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: _existing != null ? [IconButton(icon: const Icon(Icons.delete, color: Colors.red), onPressed: () => context.read<CourseController>().deleteCourse(_existing!.serverId).then((_) => Navigator.pop(context)))] : null,
      ),
      body: Form(
        key: _formKey,
          ...
          slivers: [
            SliverPadding(
              ...
              sliver: SliverFillRemaining(
                ...
                child: Column(
                  children: [
                    TextFormField(controller: _titleCtrl, decoration: InputDecoration(labelText: l10n.course_details_title)),
                    const SizedBox(height: 12),
                    
                    DropdownButtonFormField<int>(
                      value: _selectedSubjectId,
                      decoration: InputDecoration(labelText: l10n.tutor_details_subjects, prefixIcon: const Icon(Icons.school)),
                      items: subjectCtrl.subjects.map((s) => DropdownMenuItem(value: s.id, child: Text(s.name))).toList(),
                      onChanged: (val) => setState(() => _selectedSubjectId = val),
                      validator: (val) => val == null ? l10n.generic_api_error : null,
                    ),
                    TextFormField(controller: _descCtrl, decoration: InputDecoration(labelText: l10n.course_details_description), maxLines: 3),
                    TextFormField(controller: _priceCtrl, decoration: InputDecoration(labelText: l10n.course_details_price), keyboardType: TextInputType.number),
                    TextFormField(controller: _hoursCtrl, decoration: InputDecoration(labelText: l10n.course_details_hours), keyboardType: TextInputType.number),
                    TextFormField(
                      controller: _startCtrl,
                      decoration: InputDecoration(labelText: l10n.course_details_start_date, suffixIcon: const Icon(Icons.calendar_today)),
                      readOnly: true,
                      onTap: () => _selectDate(context, true, _startDate),
                    ),
                    const Spacer(),
                    SizedBox(width: double.infinity, child: ElevatedButton(onPressed: _onSave, child: Text(_existing == null ? l10n.generic_save : l10n.generic_edit))),
                  ],
                ),
              ),
            ),
